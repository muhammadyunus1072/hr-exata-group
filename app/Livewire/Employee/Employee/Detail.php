...
class Detail extends Component
{
    ...
    {
        if ($this->objId) {
            $employee = UserRepository::find((Crypt::decrypt($this->objId)));
    ...
    {
        if ($this->objId) {
            $this->redirectRoute('employee.edit', $this->objId);
        } else {
        ...
        try {
            DB::transaction(function () {
                ...
                if ($this->objId) {
                    ...
                    $user_id = Crypt::decrypt($this->objId);
                    UserRepository::update($user_id, $validateData);
                } else {
                    // Vehicle
                    $validateData = [
                    ...
                    $this->dispatch('consoleLog', $validateData);

                    $user = UserRepository::create($validateData);
                    $user_id = $user->id;
                }
